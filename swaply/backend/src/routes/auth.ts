...
export default async function authRoutes(app: FastifyInstance) {
   ...
   * on 10c — or when it turns out there was one all along, and the person signs
   * in to it (`/auth/login` folds this account into theirs). The device id is
   * generated by the client and is the only credential this account has — which
   * is why a device that has been claimed is not let back in this way. After
   * 10c the password is the credential, and a leaked device id must not be a way
   * around it.
   */
  ...
  app.post('/auth/login', async (request) => {
    ...
    // That stranger is them, and what it wished for comes along; the phone's
    // account is removed. Only after the password, only for a device nobody
    // has claimed, and never for a session the switcher minted — acting as a
    // test account and then signing in must not carry the ring into a person.
    const device =
      request.userId &&
      !request.userClaimed &&
      !request.sessionIssuedBy &&
      request.userId !== user['id']
        ? request.userId
        : null
    const carried = device ? await mergeDeviceAccount(app.db, device, user['id']) : null

    const token = await issueSession(app.db, user['id'])

    // Each wish that moved is a wish from somebody who can now give something
    // back, so it gets the search a heart gets, in the order they were
    // pressed. After the commit rather than inside it: opening a trade is a
    // transaction of its own, behind a heart as much as here. A failure costs
    // a trade the hourly sweep will open anyway, and must not cost the
    // sign-in, because the phone's old token is already gone.
    for (const itemId of carried?.likes ?? []) {
      await closeLoopThrough(app.db, user['id'], itemId).catch((err) =>
        request.log.warn({ err, itemId }, 'loop search after a merge failed'),
      )
    }
    ...
    return {
      token,
      user: publicMe(me!),
      ...(carried ? { carried: { likes: carried.likes.length } } : {}),
    }
  })
